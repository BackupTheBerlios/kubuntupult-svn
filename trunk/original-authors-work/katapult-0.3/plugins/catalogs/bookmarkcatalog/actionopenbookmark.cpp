...
{
	if(qstrcmp(item->className(), "Bookmark") == 0) {
		const Bookmark *bookmark = (const Bookmark *) item;
		new KRun(bookmark->bookmark().url());
	} else if(qstrcmp(item->className(), "MozillaBookmark") == 0) {
		const MozillaBookmark *bookmark = (const MozillaBookmark *) item;
		new KRun(bookmark->url());
	}
}

int ActionOpenBookmark::accepts(const KatapultItem *item) const
{
	return (qstrcmp(item->className(), "Bookmark") == 0 || qstrcmp(item->className(), "MozillaBookmark") == 0) ? 100 : 0;
}
