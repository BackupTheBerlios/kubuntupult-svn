 ...
 : KDialogBase(TreeList, "Configure", Ok|Cancel, Ok)
{
	...
	general->maxResults->setValue(settings->maxResults());
	...
	display->displayName->setCurrentItem(displays.findIndex(settings->displayName()));
	displayConfig = 0;
	addDisplaySettings();
...
void KatapultConfigDlg::saveSettings()
{
	...
	settings->setMaxResults(general->maxResults->value());
}
...
{
	settings->setDisplayName(display->displayName->text(index));
	addDisplaySettings();
}
